...
private Q_SLOTS:
    ...
    void initTestCase()
    {
        ...
        QVERIFY(QFileInfo("/usr/local/share/locale/en_US/LC_MESSAGES/localizedApp.mo").exists());

        QString modules(TEST_QML_DIR);
        qDebug() << modules;
        QVERIFY(QDir(modules).exists());

        view = new QQuickView;
        QQmlEngine *quickEngine = view->engine();
        UbuntuI18nPlugin::initializeContextProperties(quickEngine);

        view->setGeometry(0,0, 40, 30);
    ...
    {
        QQmlEngine engine;
        UbuntuI18nPlugin::initializeContextProperties(&engine);
        ...
        QQuickItem *root = loadTest("qml/LocalizedApp.qml");
        QVERIFY(root);
        ...
        QQuickItem* page(testItem(mainView, "column"));
        QVERIFY(page);
